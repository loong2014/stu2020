/*
 * ***********************************************************
 * File：DataBoundListAdapter.java  Module：app  Project：PaxLauncher
...
public abstract class DataBoundListAdapter<T, V extends ViewDataBinding>
    ...
    private static String TAG = DataBoundListAdapter.class.getSimpleName();
    ...
    // need auto set spm4 by item position
    private boolean needAuthSetPos = true;

    public void setNeedAuthSetPos(boolean needAuthSetPos) {
        this.needAuthSetPos = needAuthSetPos;
    }
    ...
    public void replace(List<T> update) {
        ...
        Log.v(TAG, "items count = " + String.valueOf(items == null ? 0 : items.size()));
    }

    protected abstract void bind(V binding, T item);

    protected void bind(V binding, T item, int pos) {
        bind(binding, item);
    }
